...
import { useAddReaction, useReportStory } from "@/hooks/use-stories";
import { api } from "@shared/routes";
import { z } from "zod";
import { Badge } from "@/components/ui/badge";

type Story = z.infer<typeof api.stories.list.responses[200]>[number];
  ...
  return (
    ...
    >
        ...
        <div className="flex gap-3 items-center">
          ...
          <div>
            <div className="font-medium text-foreground">
              {story.isAnonymous ? "Anonymous Friend" : "Verified User"}
            </div>
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-dashed">
        ...
        >
          <Heart className={`w-5 h-5 mr-1.5 transition-transform group-hover:scale-110 ${story.reactionsCount > 0 ? 'fill-red-100 text-red-500' : ''}`} />
          <span className="text-sm font-medium">{story.reactionsCount || 0}</span>
        </Button>
